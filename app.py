...
if not st.session_state.messages and "pending_prompt" not in st.session_state:
    ...
    cols = st.columns(3)
    sample_queries = [
        "2020年比で2030年の「死亡事故半減」は達成可能か？CRISP-DMのEvaluationフェーズとして、データと最新の法改正から重点対策を提言して。",
        "2024年の「サポカー」事故データを非サポカーと比較分析して。ドメイン知識（Layer 2）にある「残存事故バイアス」を考慮した、専門的な解釈を求む。",
        "コロナ禍（2020年）とポストコロナ（2024年）で夜間事故の傾向はどう変わった？活動量減少の影響（Layer 4）を踏まえて、件数と致死率の変化を詳しく。",
    ]
