...
2. **高密度なインサイト**: 冗長な前置きやメタ的な見出しは不要です。冒頭から核心（結論）を述べ、データに基づいた鋭い洞察を提供してください。
3. **外部ソースの明示**: `External Web Insights` で得られた情報（URLリンク）は、必ずレポートの末尾に「参照元（External Sources）」としてリスト形式で掲載してください。
4. **構造化**: 結論、視覚的データ（グラフ）、専門的解釈、統計的限界事項、および参照元を、洗練された見出しで整理してください。
5. **一気通貫の完結**: チームからの結果をすべて統合し、一回の回答でプロフェッショナルな「最終レポート」として提示してください。中途報告は厳禁です。
"""
